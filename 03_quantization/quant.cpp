...
void fp32_to_fp16(const float* src, __fp16* dst, size_t count) 
{
    for (size_t i = 0; i < (count / 8) * 8; i += 8)
    {
        ...
        vst1q_f16(dst + i, vcombine_f16(vcvt_f16_f32(src_lower), vcvt_f16_f32(src_upper)));
    }
...
void int8_to_fp32(const int8_t* src, float* dst, size_t count, float scale) 
{
    ...
    for (size_t i = 0; i < (count / 8) * 8; i += 8)
    {
        ...
        vst1q_f32(dst + i, vmulq_f32(vcvtq_f32_s32(vmovl_s16(vget_low_s16(src_chunk_16))), scale_vec));
        vst1q_f32(dst + i + 4, vmulq_f32(vcvtq_f32_s32(vmovl_s16(vget_high_s16(src_chunk_16))), scale_vec));
    }
...
void fp16_to_int8(const __fp16* src, int8_t* dst, size_t count, float scale) 
{
    ...
    {
        float16x8_t src_chunk = vld1q_f16(src + i);
        vst1_s8(dst + i, vqmovn_s16(vcvtaq_s16_f16(vdivq_f16(src_chunk, scale_vec))));
    }

    for (size_t i = (count / 8) * 8; i < count; ++i)
         dst[i] = static_cast<int8_t>(std::max(-128, std::min(127, (int)std::round(src[i] / scale))));
}
...
float fp16_max_abs(const __fp16* src, size_t count)
{
    return parallel_reduce(
        count, 
        [src](size_t i) { return std::abs(static_cast<float>(src[i])); }, 
        0.0f, 
...
void quantize_fp16_to_int8_grouped(const __fp16* src, int8_t* dst, float* scales, size_t total_elements, size_t group_size)
{
    size_t num_groups = (total_elements + group_size - 1) / group_size;

    parallel_for(num_groups, 1, [src, dst, scales, total_elements, group_size](size_t group_idx) {
        size_t group_start = group_idx * group_size;
        size_t current_group_size = std::min(group_size, total_elements - group_start);
        ...
        for (; j < current_group_size; ++j) {
            max_val = std::max(max_val, std::abs(static_cast<float>(src[group_start + j])));
        }
        
        scales[group_idx] = max_val / 127.0f;
        
        fp16_to_int8(src + group_start, dst + group_start, current_group_size, scales[group_idx]);
    });
}

void dequantize_int8_to_fp32(const int8_t* src, float* dst, float scale, size_t count)
{
    size_t elements_per_chunk = 1024;
    size_t num_chunks = (count + elements_per_chunk - 1) / elements_per_chunk;

    parallel_for(num_chunks, 1, [src, dst, scale, count, elements_per_chunk](size_t chunk_idx) {
        size_t start = chunk_idx * elements_per_chunk;
        size_t current_size = std::min(elements_per_chunk, count - start);
