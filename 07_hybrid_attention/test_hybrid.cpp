...
void test_basic_compilation() {
    std::cout << "Testing hybrid attention basic compilation..." << std::endl;
    ...
    auto K_cached = generate_random_int8(batch_size * num_kv_heads * cache_len * head_dim);
    auto V_cached = generate_random_int8(batch_size * num_kv_heads * cache_len * head_dim);
    ...
    size_t num_scale_groups = (cache_len + quant_group_size - 1) / quant_group_size;
    auto k_scales = generate_random_scales(num_scale_groups);
    auto v_scales = generate_random_scales(num_scale_groups);
